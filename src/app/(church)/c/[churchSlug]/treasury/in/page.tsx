  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Money In Records</h2>
  ...
  Audit Trail
      ...
      <form method="get" className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3 xl:grid-cols-6">
          ...
          <div>
            <label htmlFor="inflowType" className="block text-sm font-medium text-gray-700 mb-1">Type</label>
            <select id="inflowType" name="inflowType" defaultValue={inflowType} className="w-full rounded-md border border-gray-300 px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <div>
            ...
            <select id="memberId" name="memberId" defaultValue={memberId} className="w-full rounded-md border border-gray-300 px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500">
              <option value="">All</option>
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          <Link href={`/c/${churchSlug}/treasury/in`} className="rounded-md border border-gray-300 bg-white px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
            Reset
          </Link>
        </div>
      </form>
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
        {inflows.length === 0 ? (
          <div className="px-6 py-10 text-sm text-gray-600">No inflow records matched your filters.</div>
        ) : (
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Anonymous</th>
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                {inflows.map((item: any) => (
                  <tr key={item.id}>
                    <td className="px-6 py-4 text-sm text-gray-600">{item.inflow_date}</td>
                    <td className="px-6 py-4 text-sm text-gray-600">{item.inflow_type}</td>
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{Number(item.amount).toFixed(2)}</td>
                    <td className="px-6 py-4 text-sm text-gray-600">{item.is_anonymous ? "Yes" : "No"}</td>
                    ...
                    <td className="px-6 py-4 text-sm">
                      <Link href={`/c/${churchSlug}/treasury/in/${item.id}/edit`} className="text-blue-600 hover:underline">
                        Edit
                      </Link>
                      <Link
