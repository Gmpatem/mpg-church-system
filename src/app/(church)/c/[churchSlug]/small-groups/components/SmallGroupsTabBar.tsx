...
import type { SmallGroupsTabKey } from "./types";

const tabs: Array<{ key: SmallGroupsTabKey; label: string }> = [
...
export function SmallGroupsTabBar({
  activeTab,
  onChange,
}: {
  activeTab: SmallGroupsTabKey;
  onChange: (tab: SmallGroupsTabKey) => void;
}) {
  return (
    <div className="rounded-xl border border-border bg-background shadow-sm">
      <div
        role="tablist"
        aria-label="Small Groups workspace sections"
      ...
      >
        {tabs.map((tab) => {
          ...
          return (
            <button
              key={tab.key}
              id={`small-groups-tab-${tab.key}`}
              type="button"
              role="tab"
              aria-selected={isActive}
              aria-controls={`small-groups-panel-${tab.key}`}
